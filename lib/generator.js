...
function controller( name ){
  ...
  var viewName = join( 'api', 'views', inflection.underscore(name).replace('_controller','') )+'/index.html.hbs';
  var replacements = [ 
  ...
  fileData = fs.readFileSync( join( __dirname, '..', 'templates', 'generator', 'index.html.hbs' ), 'utf8' );  
  replacements.forEach( function( repl ){
    fileData = fileData.replace( repl[0], repl[1] );
  });
  ...
  console.log("       'resource /"+inflection.underscore(name).replace('_controller','')+"': '"+inflection.classify(name)+"'\n");
}
